...
def print_output(from_file: str, to_file: str, from_start: str, to_end: str, file_directory: str):
    ...
    subprocess.call(["sed", "-n", f"{int(from_start)},$p", Path(f"{file_directory}{from_file}.log").absolute()])
    subprocess.call(["printf", "\n"])
    for f in files_to_be_printed:
        subprocess.call(["cat", Path(f).absolute()])
        subprocess.call(["printf", "\n"])
    subprocess.call(["head", f"{-int(to_end)}", Path(f"{file_directory}{to_file}.log").absolute()])
